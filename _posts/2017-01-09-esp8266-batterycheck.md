...
![バッテリーチェッカー_ブレッドボード.png]({{site.baseurl}}/_posts/バッテリーチェッカー_ブレッドボード.png)
...
Arduinoのアナログポート２つを利用して、2個の電池を同時に監視するようにしました。
取得したデータはI2C通信にてESP8266側に送信を行うことにします。
なおArduino ではsprintfでfloatは使えないらしいです。その代わり、dtostrf()という関数があるのでそちらを利用しました。
